...
- The user designated `\\tustopd.trinity.local\ipv\OBTV-AI` (also `\\10.81.100.220\ipv\OBTV-AI`) for retained analysis files. Keep active PostgreSQL/vector-search data on local NVMe and playback sources in their existing SMB locations. This output folder must remain outside media-discovery scanning. A disposable uid-0 container test verified root-folder write/read/rename/delete; child-folder mount rollout and any quota still need validation/configuration.
...
- Archive mode is implemented but disabled by default (`CURATOR_ARCHIVE_MODE=0`). When enabled for valid SMB source roots, browser playback uses RAM-only HLS segments with sidecar audio, not retained MP4s. Segments transcode on CPU; real SMB playback concurrency still needs production benchmarking. Legacy/nonarchive behavior remains unchanged.
...
- Missing/invalid cutoff dates are persisted for review, never substituted. Admin bounded discovery/admission controls live on `/curator`; CLI and explicitly opt-in recurrence are documented in `docs/curator-catalog.md`. Dry-run persists inventory/checkpoints but queues no processing. Production ingestion/recurrence and proxy deletion remain off pending live canaries; code availability is not evidence of deployment.
