...
- Missing/invalid cutoff dates are persisted for review, never substituted. Admin bounded discovery/admission controls live on `/curator`; CLI and explicitly opt-in recurrence are documented in `docs/curator-catalog.md`. Dry-run persists inventory/checkpoints but queues no processing. Production video, audio and image canaries completed successfully with no local proxy creation. Bulk ingestion and recurrence remain off: physical GPU0 had memory contention with existing services, so canaries required the available GPU. Existing proxies remain untouched.
