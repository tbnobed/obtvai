...
- Live voice dictation uses `/api/speech/transcribe?partial=true`: complete 16 kHz WAV snapshots about every 2 seconds, one request at a time, up to 60 seconds. Interim text replaces the owned dictated range; Stop sends a final refinement, Cancel restores the pre-dictation text, and manual editing stops dictation without overwriting edits. Local multilingual Whisper small runs on CPU in a killable cached child process; disconnected requests cancel inference. Microphone access requires HTTPS; first use downloads the public model into the existing model cache. No external speech API key is needed. The mock preview explicitly returns 503 rather than inventing a transcript; use the deployed API for real microphone transcription. Dictation never auto-submits.
...
- Cloned-voice dubbing and the multilingual Voice Generator use pinned Chatterbox V3: Latin American Spanish for `es`, general multilingual V3 otherwise, with the matching V3 decoder. The official language-pack runtime is isolated under `/opt/chatterbox-v3` to protect the existing Torch/Turbo stack. Model files download to the shared cache on first use (~3 GB per variant). V3 failures are explicit, never automatic XTTS substitution; `DUB_ENGINE=xtts` remains an explicit opt-out for dubbing. Existing published dubs are not regenerated by an upgrade.
...
- Curator SendToOBTV ingest: the watcher monitors only the writable `/curator-inbox` for plugin `<assets>` XML, resolves each exact `WebProxyPath` beneath the read-only `/curator` mount, and calls the durable `/media/curator-import` endpoint; completed/terminal manifests move to `processed/` or `failed/`. Broad `/curator` scanning is off by default and only enabled with `CURATOR_LEGACY_FOLDER_WATCH=1` for backfills. Direct proxy ingest still self-links; hi-res source discovery/reconstruction remains unchanged. `/curator` must be mounted in api, watcher, and ALL media workers (gpu, gpu-2, cpu); `/curator-inbox` only needs a writable watcher mount.
...
## Archive ingestion implementation and rollout

- The user designated `\\tustopd.trinity.local\ipv\OBTV-AI` (also `\\10.81.100.220\ipv\OBTV-AI`) for retained analysis files. Keep active PostgreSQL/vector-search data on local NVMe and playback sources in their existing SMB locations. This output folder must remain outside media-discovery scanning. Production child-folder mounts and API/all-worker CIFS read/write probes were validated during the September 24, 2026 rollout; any folder quota still needs confirmation.
- Use Curator `IngestCompleteDate`, not production/air/file dates. The long-term plan/default remains inclusive January 1, 2023; the user narrowed the active production test bed to **January 1, 2025 onward**, via `CURATOR_CATALOG_CUTOFF=2025-01-01`. Older indexed assets are preserved; new admissions recheck the current policy.
...
- Archive mode defaults off in source (`CURATOR_ARCHIVE_MODE=0`) but was enabled in production on September 24, 2026. Browser playback uses RAM-only HLS segments with sidecar audio, not retained MP4s. Real browser playback/seeking and concurrent segment requests passed; multi-user capacity still needs benchmarking. Segments transcode on CPU. Legacy/nonarchive behavior remains unchanged.
- The optional `archive-artifacts.compose.yml` binds seven derived-output child directories onto SMB while preserving the local `/artifacts` volume and legacy proxies. Follow `docs/archive-artifacts-storage.md` for preflight, dry-run copy, maintenance-window migration and rollout. Never copy old proxies to IPV or delete them as part of this cutover.
- Missing/invalid cutoff dates are persisted for review. Admin controls/runner heartbeat live on `/curator`; operational commands are in `docs/curator-catalog.md`. The opt-in Compose `catalog-runner` service now runs the production 2025 test bed every 60 seconds, max one admission and one inflight asset, with storage guards and a durable failure pause. Discovery/admission rotate all three asset types. Physical GPU0 remains occupied by existing services, so `GPU_SECONDARY_QUEUES=gpu-secondary` puts worker-gpu-2 on an explicit standby queue; ordinary GPU jobs run on worker-gpu/physical GPU1 without skipping analysis stages. Existing proxies remain untouched.
- Recovery policy: isolated asset failures have three total pipeline attempts, then remain quarantined (`failed`) while other assets continue. Reconcile successful manual retries before replaying anything. Shared infrastructure/storage failures pause immediately; three distinct GPU-failed pipeline attempts within 30 minutes also pause. Failure accounting persists across manual/dry/recurring observers. Concurrent manual reruns of identical ingest stages still lack explicit root-attempt attribution; do not treat their timestamps alone as proof of ownership.
