...
_SELECT_SYSTEM = (
    ...
    "to them. Do not reject them for lacking dialog. The quoted text in a "
    "visual-match snippet is the SEARCH PHRASE that matched — it does NOT "
    "identify who or what is on screen. If the phrase names a person or song, "
    "the footage merely LOOKS like that description; treat it as an unnamed "
    "performance. Never claim specific artists, songs, or people appear in "
    "the cut based on these labels, and never describe visual matches as "
    "different performers just because their labels differ.\n"
...
async def export_cut(project_id: str, body: dict, db: AsyncSession = Depends(get_db)):
    ...
    for mid, fname, op, sp, cid, fps_v, cfolder, dsec in rows.all():
        fnames[mid] = fname
        if dsec:
    ...
    if fmt == "xmeml":
        ...
        content, filename = _xmeml(name, ns_clips, paths, lognotes, fps_map, audio_map, folders, durations), f"{safe}.xml"
    elif fmt == "fcpxml":
