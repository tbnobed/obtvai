...
def run_qc(self, media_id: str, job_id: str = None):
    ...
    try:
        ...
        audio_out = _run_ffmpeg_filter(src_path, ["-af", "volumedetect", "-vn"])
        max_vol = _grep_float(audio_out, r"max_volume:\s*(-?[\d.]+)\s*dB")
        mean_vol = _grep_float(audio_out, r"mean_volume:\s*(-?[\d.]+)\s*dB")
